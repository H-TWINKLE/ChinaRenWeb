			...
			<div class="page-title hidden-print">
				<div>
					...
					<ul class="breadcrumb side">
						...
						<li>提问管理</li>
						<li class="active"><a href="#">所有提问</a></li>
					</ul>
				</div>
			</div>
			<div class="row">
								...
								<tbody>
									<tr>
										<%
											...
											try {
												list = ManageFactory.getInstance().findAllQues(0,999);
										...
										<%
											Iterator<UserQues> iterator = list.iterator();
											while (iterator.hasNext()) {
												UserQues pe = iterator.next();
										%>
										<td align="center"><%=pe.getQ_id() %></td>
										<td><%=pe.getU_id().getName()%></td>
										<td><%=pe.getTitle()%></td>
										<td><%=pe.getContent()%></td>
										<td align="center"><a onclick="update(<%=pe.getQ_id()%>)">删除提问</a></td>
									</tr>
									<%
										}
									%>
				...
				$.ajax({
					...
					success : function(data) {
						if (data == "删除成功！") {
							...
							window.setTimeout(
									"window.location='chinaren-managequestion.jsp'",
									1000);
						} else {
							$.notify({
								title : "提示 : ",
